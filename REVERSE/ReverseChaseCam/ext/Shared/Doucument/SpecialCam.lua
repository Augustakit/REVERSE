...
-- somewhere inside ModifyBlueprint()
local targetEntity
if config.clone_or_entity_index then
   targetEntity = CloneOrEntity(config.clone_or_entity_index) -- Handle cloning and related code inside a seperate function
else
   targetEntity = vehicleEntity.components[config.or_entity_index]
end
