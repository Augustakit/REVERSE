...
local VEHICLE_CONFIG = require('__shared/Config/ArtilleryConfig')
...
Events:Subscribe('Level:RegisterEntityResources', function()
  for _, config in pairs(VEHICLE_CONFIG) do
    ...
    if partition ~= nil then
      ModifyArtilleryBlueprint(partition.primaryInstance, config)
    end
  end
end)
...
function ModifyArtilleryBlueprint(instance, config)
  ...
  vehicleBlueprint.propertyConnections:erase(config.ERASE_INDEX)

    local orEntity = OrEntityData(vehicleEntity.components[config.Or_ClONE])
  ConnectionHelper:AddPropertyConnection(vehicleBlueprint,
    andEntity, 'Out',
    orEntity, 'In2' )
